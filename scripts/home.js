...
function parseTestVersion(version) {
    ...
    if (!match) throw new Error(`测试服版本格式错误：${version}`);
    return { major: Number(match[1]), minor: Number(match[2]), patch: Number(match[3]) };
}

function addMs(date, ms) { return new Date(date.getTime() + ms); }
function addDays(date, days) { return addMs(date, days * DAY_MS); }
...
function getMainVersionRank(version) {
    ...
    if (!match) throw new Error(`主版本格式错误：${version}`);
    return Number(match[1]) * 100 + Number(match[2]);
}

function getTestServerShiftDays(version) {
    const shiftFrom = homeData.testServer.shiftFrom;
    if (!shiftFrom || getMainVersionRank(version) < getMainVersionRank(shiftFrom)) return 0;
...
function getCharacterAgeHtml(name, birthdayStamp, color = "#DD0000") {
    ...
    if (age % 100 === 0) age = `<color style='color:${color}'>${age}</color>`;
...
function getFutureDateEntries(panelData, now = new Date()) {
    ...
    let result = entries
        .map((entry) => ({ ...entry, date: new Date(entry.date.getTime() + displayShiftMs) }))
        .map((entry) => ({ ...entry, stamp: entry.date.getTime() }))
        .sort((a, b) => a.stamp - b.stamp);

    if (panelData.start_from) {
        const startIndex = result.findIndex(
            (entry) => entry.version === panelData.start_from.version && entry.kind === panelData.start_from.kind
        );
        if (startIndex >= 0) result = result.slice(startIndex);
    }
...
function formatRemaining(ms) {
    ...
    if (days > 0) return `${String(days).padStart(2, "0")}:${time}`;
    return time;
}
...
function renderGameTabs() {
    renderList(byId("gameTabs"), homeData.gameTabs, (tab) => {
        ...
        return element("a", {
            ...
            attrs: { ...noNavigationAttrs(tab), "data-game-key": tab.key },
        });
    });
}
...
function renderActions() {
    ...
    const gameKey = activeTab?.key === "future" ? "sr" : (activeTab?.key ?? "zzz");
...
function renderAttrIcons(item) {
    const icons = [];
    if (item.element) icons.push(image(iconPaths.elements[item.element], "featured-icon", item.element));
    if (item.path) icons.push(image(iconPaths.paths[item.path], "featured-icon", item.path));
    return icons;
}
...
function renderFeaturedCard(item, options) {
    return element("a", {
        ...
        children: [
            element("span", { className: "featured-version", text: options.version }),
            element("span", { className: "featured-image-wrap", children: [image(options.imagePath, "featured-image", item.name)] }),
            element("span", { className: "featured-name", text: item.name }),
            element("span", { className: "featured-icons", children: renderAttrIcons(item) }),
        ],
    });
}
...
function renderFuturePanel() {
    ...
    const now = new Date();
    panel.replaceChildren(...homeData.futureDate.panels.map((panelData) => renderFutureDatePanel(panelData, now)));
}
...
function renderFutureDatePanel(panelData, now) {
    const rows = getFutureDateEntries(panelData, now).map((entry) =>
        element("tr", {
            children: [
                element("td", { text: formatHyphenDate(entry.date), style: { textAlign: "right" } }),
                element("td", { className: "fntd_time", text: formatFutureRemaining(entry.stamp - now.getTime()), attrs: { "data-id": entry.stamp }, style: { textAlign: "center" } }),
                element("td", { html: entry.html }),
            ],
        })
    );
    ...
    return element("div", {
        ...
        children: [
            element("p", { className: "dir_head", text: panelData.title, style: { margin: "15px 5px 0" } }),
            element("p", { className: `${panelData.key === "sr" ? "c2_" : "c1_"} dir_subhead` }),
            element("div", {
                className: "age",
                children: [
                    element("div", { className: `age_sub_1 table_${panelData.key}_1`, children: [image(panelData.emote, "", panelData.birthday_name)] }),
                    element("div", { className: `age_sub_2 table_${panelData.key}_2`, children: [element("p", { html: getCharacterAgeHtml(panelData.birthday_name, panelData.birthday_stamp, "#FF9999") })] }),
                ],
            }),
            element("table", { className: `ctable table_${panelData.key}`, children: [element("tbody", { children: rows })] }),
        ],
    });
}
...
function renderTestCountdown() {
    ...
    container.replaceChildren(
        element("div", {
            ...
            children: [
                element("div", { className: "cntd_emote", children: [image("./images/emote/Yunli/1.png", "", "云璃")] }),
                element("div", { className: "c_a_w", children: [element("p", { className: "c2_a", html: getYunliAgeHtml() })] }),
            ],
        }),
        element("p", { className: "countdown c2 c2_b", text: `${timeline.next.version} ${formatRemaining(timeline.remainingMs)}` })
    );
    if (!byId("futurePanel").hidden) renderFuturePanel();
}
